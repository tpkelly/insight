...
if (window._phantom) {
    console.log = function() {
        phantom.sendMessage('verbose', Array.prototype.slice.apply(arguments)
            .join(', '));
    };
}
...
(function() {
    ...
    PhantomReporter.prototype.reportRunnerResults = function(runner) {
        this.finished = true;
        var specIds = map(runner.specs(), function(a) {
            return a.id;
        });
    ...
    PhantomReporter.prototype.reportSpecResults = function(spec) {
        spec.duration = (new Date())
            .getTime() - spec.startTime;
    ...
    jasmine.getEnv()
        .addReporter(new PhantomReporter());
}());
