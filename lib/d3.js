! function() {
    ...
    d3_selectionPrototype.select = function(selector) {
        ...
        for (var j = -1, m = this.length; ++j < m;) {
            ...
            subgroup.parentNode = (group = this[j])
                .parentNode;
    ...
    d3_selectionPrototype.classed = function(name, value) {
        if (arguments.length < 2) {
            if (typeof name === "string") {
                var node = this.node(),
                    n = (name = d3_selection_classes(name))
                    .length,
                    i = -1;
                if (value = node.classList) {
                    ...
                    while (++i < n)
                        if (!d3_selection_classedRe(name[i])
    ...
    function d3_selection_classed(name, value) {
        name = d3_selection_classes(name)
    ...
    d3_selectionPrototype.style = function(name, value, priority) {
        ...
        if (n < 3) {
            ...
            if (n < 2) return d3_window.getComputedStyle(this.node(), null)
    ...
    d3_selectionPrototype.data = function(value, key) {
        var i = -1,
            ...
            group, node;
        if (!arguments.length) {
            value = new Array(n = (group = this[0])
                .length);
            while (++i < n) {
                if (node = group[i]) {
    ...
    d3.touches = function(container, touches) {
        if (arguments.length < 2) touches = d3_eventSource()
            .touches;
        return touches ? d3_array(touches)
            .map(function(touch) {
        ...
        function dragstart(id, position, subject, move, end) {
            return function() {
                var that = this,
                    ...
                    dragOffset, dragSubject = d3.select(subject())
                ...
                function ended() {
                    ...
                    dragSubject.on(move + dragName, null)
    ...
    d3.behavior.zoom = function() {
        ...
        function rescale() {
            if (x1) x1.domain(x0.range()
                .map(function(x) {
                    return (x - view.x) / view.k;
                })
                .map(x0.invert));
            if (y1) y1.domain(y0.range()
                .map(function(y) {
        ...
        function mousedowned() {
            ...
            function ended() {
                subject.on(mousemove, d3_window === that ? mousewheelreset : null)
                    .on(mouseup, null);
                dragRestore(dragged && d3.event.target === target);
                zoomended(dispatch);
            }
        ...
        function touchstarted() {
            ...
            function ended() {
                ...
                subject.on(mousedown, mousedowned)
                    .on(touchstart, touchstarted);
                dragRestore();
                zoomended(dispatch);
            }
    ...
    d3.hcl = function(h, c, l) {
        return arguments.length === 1 ? h instanceof d3_Hcl ? d3_hcl(h.h, h.c, h.l) : h instanceof d3_Lab ? d3_lab_hcl(h.l, h.a, h.b) : d3_lab_hcl((h = d3_rgb_lab((h = d3.rgb(h))
                .r, h.g, h.b))
            .l, h.a, h.b) : d3_hcl(+h, +c, +l);
    };
    ...
    d3_hclPrototype.rgb = function() {
        return d3_hcl_lab(this.h, this.c, this.l)
            .rgb();
    };
    ...
    d3.lab = function(l, a, b) {
        return arguments.length === 1 ? l instanceof d3_Lab ? d3_lab(l.l, l.a, l.b) : l instanceof d3_Hcl ? d3_hcl_lab(l.l, l.c, l.h) : d3_rgb_lab((l = d3.rgb(l))
            .r, l.g, l.b) : d3_lab(+l, +a, +b);
    };
    ...
    function d3_rgb_hex(v) {
        return v < 16 ? "0" + Math.max(0, v)
            .toString(16) : Math.min(255, v)
            .toString(16);
    }
    ...
    function d3_xhr(url, mimeType, response, callback) {
        ...
        xhr.send = function(method, data, callback) {
            ...
            if (callback != null) xhr.on("error", callback)
    ...
    d3.touch = function(container, touches, identifier) {
        if (arguments.length < 3) identifier = touches, touches = d3_eventSource()
            .changedTouches;
        if (touches)
        ...
        return function(specifier) {
            ...
            return function(value) {
                ...
                var length = prefix.length + before.length + after.length + (zcomma ? 0 : negative.length),
                    padding = length < width ? new Array(length = width - length + 1)
                    .join(fill) : "";
    ...
    var d3_format_types = d3.map({
        ...
        r: function(x, p) {
            return (x = d3.round(x, d3_format_precision(x, p)))
                .toFixed(Math.max(0, Math.min(20, d3_format_precision(x * (1 + 1e-15), p))));
        }
    });
    ...
    ["sunday", "monday", "tuesday", "wednesday", "thursday", "friday", "saturday"].forEach(function(day, i) {
        ...
        var interval = d3_time[day] = d3_time_interval(function(date) {
            (date = d3_time.day(date))
                .setDate(date.getDate() - (date.getDay() + i) % 7);
            return date;
        }, function(date, offset) {
            date.setDate(date.getDate() + Math.floor(offset) * 7);
        }, function(date) {
            var day = d3_time.year(date)
                .getDay();
    ...
    function d3_locale_timeFormat(locale) {
        ...
        function d3_time_parseAmPm(date, string, i) {
            var n = d3_time_periodLookup.get(string.substring(i, i += 2)
                .toLowerCase());
    ...
    function d3_time_formatPad(value, fill, width) {
        ...
        return sign + (length < width ? new Array(width - length + 1)
            .join(fill) + string : string);
    }

    function d3_time_formatRe(names) {
        return new RegExp("^(?:" + names.map(d3.requote)
            .join("|") + ")", "i");
    }
    ...
    d3.geo.albersUsa = function() {
        ...
        albersUsa.invert = function(coordinates) {
            ...
            return (y >= .12 && y < .234 && x >= -.425 && x < -.214 ? alaska : y >= .166 && y < .234 && x >= -.214 && x < -.115 ? hawaii : lower48)
                .invert(coordinates);
        };
    ...
    d3.geo.graticule = function() {
        ...
        function lines() {
            return d3.range(Math.ceil(X0 / DX) * DX, X1, DX)
                .map(X)
                .concat(d3.range(Math.ceil(Y0 / DY) * DY, Y1, DY)
                    .map(Y))
                .concat(d3.range(Math.ceil(x0 / dx) * dx, x1, dx)
                    .filter(function(x) {
                        return abs(x % DX) > ε;
                    })
                    .map(x))
        ...
        graticule.extent = function(_) {
            ...
            return graticule.majorExtent(_)
                .minorExtent(_);
        };
    ...
    function d3_geom_voronoi(sites, bbox) {
        var site = sites.sort(d3_geom_voronoiVertexOrder)
            .pop(),
            x0, y0, circle;
    ...
    d3.geom.voronoi = function(points) {
        ...
        function voronoi(data) {
            var polygons = new Array(data.length),
                ...
                x1 = clipExtent[1][0],
                y1 = clipExtent[1][1];
            d3_geom_voronoi(sites(data), clipExtent)
    ...
    function d3_scale_linear(domain, range, interpolate, clamp) {
        ...
        scale.rangeRound = function(x) {
            return scale.range(x)
                .interpolate(d3_interpolateRound);
        };
    ...
    d3.scale.log = function() {
        return d3_scale_log(d3.scale.linear()
            .domain([0, 1]), 10, true, [1, 10]);
    };
    ...
    function d3_scale_log(linear, base, positive, domain) {
        ...
        scale.domain = function(x) {
            if (!arguments.length) return domain;
            positive = x[0] >= 0;
            linear.domain((domain = x.map(Number))
                .map(log));
            return scale;
        };
        ...
        scale.tickFormat = function(n, format) {
            ...
            var k = Math.max(.1, n / scale.ticks()
                    .length),
    ...
    function d3_svg_line(projection) {
        ...
        line.interpolate = function(_) {
            ...
            else interpolateKey = (interpolate = d3_svg_lineInterpolators.get(_) || d3_svg_lineLinear)
                .key;
            return line;
        };
    ...
    d3_transitionPrototype.tween = function(name, tween) {
        ...
        if (arguments.length < 2) return this.node()
            .__transition__[id].tween.get(name);
    ...
    d3_transitionPrototype.styleTween = function(name, tween, priority) {
        ...
        function styleTween(d, i) {
            var f = tween.call(this, d, i, d3_window.getComputedStyle(this, null)
                .getPropertyValue(name));
            return f && function(t) {
        ...
        function axis(g) {
            g.each(function() {
                ...
                var ticks = tickValues == null ? scale1.ticks ? scale1.ticks.apply(scale1, tickArguments_) : scale1.domain() : tickValues,
                    ...
                    .attr("class", "tick")
                    .style("opacity", ε),
                    tickExit = d3.transition(tick.exit())
                    .style("opacity", ε)
                    .remove(),
                    tickUpdate = d3.transition(tick.order())
                    .style("opacity", 1),
                    tickTransform;
                var range = d3_scaleRange(scale1),
                    path = g.selectAll(".domain")
                    .data([0]),
                    pathUpdate = (path.enter()
                        .append("path")
                        .attr("class", "domain"),
                        d3.transition(path));
    ...
    d3.svg.brush = function() {
        ...
        function brush(g) {
            g.each(function() {
                ...
                if (x) {
                    ...
                    backgroundUpdate.attr("x", range[0])
                        .attr("width", range[1] - range[0]);
                    redrawX(gUpdate);
                }
                if (y) {
                    range = d3_scaleRange(y);
                    backgroundUpdate.attr("y", range[0])
        ...
        function redrawX(g) {
            g.select(".extent")
                .attr("x", xExtent[0]);
            g.selectAll(".extent,.n>rect,.s>rect")
        ...
        function redrawY(g) {
            g.select(".extent")
                .attr("y", yExtent[0]);
            g.selectAll(".extent,.e>rect,.w>rect")
        ...
        function brushstart() {
            ...
            if (d3.event.changedTouches) {
                w.on("touchmove.brush", brushmove)
            ...
            g.style("pointer-events", "none")
                ...
                .style("display", null);
            d3.select("body")
                .style("cursor", eventTarget.style("cursor"));
            event_({
                type: "brushstart"
            });
            brushmove();
    ...
    function d3_time_scale(linear, methods, format) {
        ...
        scale.nice = function(interval, skip) {
            ...
            function skipped(date) {
                return !isNaN(date) && !interval.range(date, d3_time_scaleDate(+date + 1), skip)
                    .length;
            }
    ...
    var d3_time_scaleMilliseconds = {
        range: function(start, stop, step) {
            return d3.range(Math.ceil(start / step) * step, +stop, step)
                .map(d3_time_scaleDate);
        },
        floor: d3_identity,
        ceil: d3_identity
