    ...
    var trim = (function() {
        ...
        if (!String.prototype.trim) {
            return function(value) {
                return isString(value) ? value.replace(/^\s\s*/, '')
                    .replace(/\s\s*$/, '') : value;
            };
        }
        return function(value) {
    ...
    var JQLitePrototype = JQLite.prototype = {
        ready: function(fn) {
            ...
            if (document.readyState === 'complete') {
                ...
                // jshint -W064
                JQLite(window)
                    .on('load', trigger); // fallback to window.onload for others
                // jshint +W064
            }
        },
        toString: function() {
    ...
    forEach({
        ...
        scope: function(element) {
            ...
            return jqLite(element)
                .data('$scope') || jqLiteInheritedData(element.parentNode || element, ['$isolateScope', '$scope']);
        },

        isolateScope: function(element) {
            // Can't use jqLiteData here directly so we stay compatible with jQuery!
            return jqLite(element)
                .data('$isolateScope') || jqLite(element)
            ...
            if (BOOLEAN_ATTR[lowercasedName]) {
                if (isDefined(value)) {
                    ...
                    return (element[name] ||
                        ...
                        .specified) ? lowercasedName : undefined;
                }
    ...
    function createInjector(modulesToLoad) {
        ...
        function createInternalInjector(cache, factory) {
            ...
            function instantiate(Type, locals) {
                ...
                Constructor.prototype = (isArray(Type) ? Type[Type.length - 1] : Type)
                    .prototype;
    ...
    function Browser(window, document, $log, $sniffer) {
        ...
        self.onUrlChange = function(callback) {
            ...
            if (!urlChangeInit) {
                ...
                if ($sniffer.history) jqLite(window)
                    .on('popstate', fireUrlChange);
                // hashchange event
                if ($sniffer.hashchange) jqLite(window)
                    .on('hashchange', fireUrlChange);
                // polling
       ...
       */
                        ...
                        case 1:
                            ...
                            addDirective(directives,
                                directiveNormalize(nodeName_(node)
                                    .toLowerCase()), 'E', maxPriority, ignoreDirective);
                ...
                function replaceWith($rootElement, elementsToRemove, newNode) {
                    ...
                    for (var k = 1, kk = elementsToRemove.length; k < kk; k++) {
                        ...
                        jqLite(element)
                            .remove(); // must do this way to clean up expando
    ...
    function createHttpBackend($browser, createXhr, $browserDefer, callbacks, rawDocument) {
        ...
        return function(method, url, post, callback, headers, timeout, withCredentials, responseType) {
            ...
            if (lowercase(method) == 'jsonp') {
                var callbackId = '_' + (callbacks.counter++)
                    .toString(36);
            ...
            function completeRequest(callback, status, response, headersString, statusText) {
                ...
                if (status === 0) {
                    status = response ? 200 : urlResolve(url)
                        .protocol == 'file' ? 404 : 0;
                }
 ...
 */
    ...
    function stripFile(url) {
        return url.substr(0, stripHash(url)
            .lastIndexOf('/') + 1);
    }
            ...
            function($rootScope, $browser, $sniffer, $rootElement) {
                ...
                $rootElement.on('click', function(event) {
                    ...
                    if (isObject(absHref) && absHref.toString() === '[object SVGAnimatedString]') {
                        ...
                        // an animation.
                        absHref = urlResolve(absHref.animVal)
                            .href;
                    }
            ...
            nextTick(function() {
                ref(value)
                    .then(function(value) {
                        ...
                        result.resolve(ref(value)
                            .then(wrappedCallback, wrappedErrback, wrappedProgressback));
                    }, function(reason) {
                        if (done) return;
                        done = true;
                    ...
                    for (var prop in bodyStyle) {
                        if (match = vendorRegex.exec(prop)) {
                            ...
                            vendorPrefix = vendorPrefix.substr(0, 1)
                                .toUpperCase() + vendorPrefix.substr(1);
                            break;
                        }
                    }
                ...
                compile: function(element, attr) {
                    ...
                    return function(scope, element, attr) {
                        var selectCtrlName = '$selectController',
                            ...
                            parent.parent()
                            .data(selectCtrlName); // in case we are in optgroup
...
!angular.$$csp() && angular.element(document)
    .find('head')
    .prepend('<style type="text/css">@charset "UTF-8";[ng\\:cloak],[ng-cloak],[data-ng-cloak],[x-ng-cloak],.ng-cloak,.x-ng-cloak,.ng-hide{display:none !important;}ng\\:form{display:block;}.ng-animate-block-transitions{transition:0s all!important;-webkit-transition:0s all!important;}</style>');
